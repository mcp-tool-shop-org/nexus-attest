...
from nexus_attest.attestation.xrpl.client import XRPLClient
from nexus_attest.attestation.xrpl.errors import (
...
def plan(
    ...
    *,
    ...
    if not validate_memo_size(payload_bytes):
        raise ValueError(
            f"memo payload exceeds {MAX_MEMO_BYTES} bytes (got {len(payload_bytes)} bytes)"
        )
